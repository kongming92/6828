...
sys_page_alloc(envid_t envid, void *va, int perm)
{
	...
	// LAB 4: Your code here.
	// case 2: va >= UTOP or not page aligned
	if (((size_t)va >= UTOP) || (((size_t)va % PGSIZE) != 0)) {
		return -E_INVAL;
	}
	// case 3: perm is inappropriate
	...
	if ((err = page_insert(env->env_pgdir, pinfo, va, perm)) < 0) {
		page_free(pinfo);
		return err;
	}
	return 0;
	     ...
	     envid_t dstenvid, void *dstva, int perm)
{
	...
	if ((err = page_insert(dstenv->env_pgdir, pinfo, dstva, perm)) < 0) {
		return err;
	}
	return 0;
}
...
sys_ipc_try_send(envid_t envid, uint32_t value, void *srcva, unsigned perm)
{
	...
	if ((size_t)srcva < UTOP) {
		...
		pte_t *srcpte;
		if ((pinfo = page_lookup(curenv->env_pgdir, srcva, &srcpte)) == NULL) {
			return -E_INVAL;
		}
		if ((perm & PTE_W) && !(*srcpte & PTE_W)) {
			return -E_INVAL;
	...
	if ((size_t)srcva < UTOP && (size_t)(dstenv->env_ipc_dstva) < UTOP) {
		if (page_insert(dstenv->env_pgdir, pinfo, dstenv->env_ipc_dstva, perm) < 0) {
			return -E_NO_MEM;
		} else {
...
static int sys_net_transmit(char *buf, int len) {
	if ((size_t)buf < UTOP) {
		return e1000_tx_pkt(buf, len);
	}
	return -E_INVAL;
}
